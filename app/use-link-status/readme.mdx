...
<FlowDiagram
  ...
  steps={['Click', 'Pending state', 'URL update', 'Loading UI']}
  note="`useLinkStatus` covers the pending phase before the URL changes."
/>
...
| Topic    | Guidance                                            |
| -------- | --------------------------------------------------- |
| Best use | When prefetching is off or dynamic routes are slow  |
| UX       | Keep indicators subtle and close to the interaction |
| Pairing  | Combine with `loading.js` for full coverage         |
...
- [Code](https://github.com/kewonit/The-App-Router-Gallery/tree/main/app/use-link-status)
