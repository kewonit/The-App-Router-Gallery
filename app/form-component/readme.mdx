...
<FlowDiagram
  title="Form → Action → UI"
  steps={["User submit", "Server Action", "Revalidate", "UI update"]}
  note="`next/form` pairs well with Server Actions for low-friction mutations."
/>

### Pitfalls & Tips

- Validate on the server for authoritative results.
- Use `useActionState` to surface field-level errors.
