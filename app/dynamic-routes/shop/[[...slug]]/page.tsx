  ...
  return (
    <Boundary label="shop/[[...slug]]/page.tsx" animateRerendering={false}>
      <div className="flex flex-col gap-6">
        {/* Params display */}
        <div className="flex flex-col gap-2 rounded-lg border border-orange-300/50 bg-orange-100/50 p-4 dark:border-orange-900/50 dark:bg-orange-950/20">
          ...
          <Link
            ...
            className={
              segments.length === 0
                ...
                : 'text-gray-600 transition-colors hover:text-gray-900 dark:text-gray-500 dark:hover:text-gray-300'
            }
          >
            Shop
          </Link>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {Object.entries(categories).map(([key, category]) => (
          <Link
            ...
            className="group rounded-lg border border-gray-200 bg-gray-100/50 p-4 transition-colors hover:border-gray-300 dark:border-gray-800 dark:bg-gray-900/30 dark:hover:border-gray-700"
          >
            <h3 className="font-medium text-gray-700 group-hover:text-gray-900 dark:text-gray-300 dark:group-hover:text-white">
              {category.name}
            </h3>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {cat.items.map((item) => (
          <Link
            ...
            className="group rounded-lg border border-gray-200 bg-gray-100/50 p-3 transition-colors hover:border-gray-300 dark:border-gray-800 dark:bg-gray-900/30 dark:hover:border-gray-700"
          >
            <span className="text-sm font-medium text-gray-700 capitalize group-hover:text-gray-900 dark:text-gray-300 dark:group-hover:text-white">
              {item}
            </span>
          </Link>
        ))}
...
function ProductView({ segments }: { segments: string[] }) {
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-lg border border-green-300/50 bg-green-100/50 p-4 dark:border-green-900/50 dark:bg-green-950/20">
        ...
        <ul className="mt-2 flex flex-col gap-1">
          {segments.map((segment, index) => (
            <li key={index} className="flex items-center gap-2 text-xs">
              <span className="flex size-5 items-center justify-center rounded-full bg-gray-200 text-gray-600 dark:bg-gray-800 dark:text-gray-400">
                {index + 1}
              </span>
