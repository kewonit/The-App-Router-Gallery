  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        {products.map((product) => (
          <div key={product.name} className="flex flex-col gap-2">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-gray-600">200×200 • quality: 85</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-lg border border-gray-800 bg-gray-900/30 p-3 text-xs text-gray-500">
        <strong className="text-gray-400">Note:</strong> Images are
        automatically served in WebP or AVIF format when supported by the
        browser, reducing file size by ~30-50%.
      </div>
    </div>
  );
}
